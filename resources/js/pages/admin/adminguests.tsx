...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Huéspedes', href: '/admin/huespedes' },
];
...
export default function AdminGuests() {
  const { users, errors: serverErrors, flash,pagination } = usePage<PageProps>().props;
  const usersTyped = users as Pagination<Guest>;

  // Estados principales
  const [guests, setGuests] = useState<Guest[]>(usersTyped.data);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editingGuest) {
      Inertia.put(`/admin/huespedes/${editingGuest.id}`, formData, {
        onSuccess: () => {
          closeModal();
          setIsLoading(false);
        },
      ...
      Inertia.post('/admin/huespedes', formData, {
        onSuccess: () => {
          closeModal();
          setIsLoading(false);
        },
  ...
  function handleDelete(id: number) {
    if (confirm('¿Estás seguro de eliminar este huésped?')) {
      setAnimatingCard(id);
      setIsLoading(true);
      Inertia.delete(`/admin/huespedes/${id}`, {
        onSuccess: () => {
          setIsLoading(false);
              ...
              <div className="flex items-center space-x-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-lg">
                    Administra y controla la información de tus huéspedes de manera profesional
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
              <div className="bg-gradient-to-br from-emerald-500/10 to-green-600/10 dark:from-emerald-500/20 dark:to-green-600/20 rounded-2xl p-6 border border-emerald-200 dark:border-emerald-800">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400">Total Huéspedes</p>
                    <p className="text-3xl font-bold text-emerald-700 dark:text-emerald-300">{guests.length}</p>
                  </div>
              ...
              <div className="bg-gradient-to-br from-blue-500/10 to-cyan-600/10 dark:from-blue-500/20 dark:to-cyan-600/20 rounded-2xl p-6 border border-blue-200 dark:border-blue-800">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-blue-700 dark:text-blue-300">{filteredAndSortedGuests.length}</p>
                  </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700">
                  <tr>
                    {[
                      { key: 'name', label: 'Huésped', icon: User },
              ...
              {filteredAndSortedGuests.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                    {search ? 'No se encontraron huéspedes' : 'Aún no tienes huéspedes'}
                  </h3>
